...
function showDetailScreen(deviceId) {
    ...
    detailContentDiv.innerHTML = `
        <div class="alarm-card">
            ...
            <div class="message-box" id="detail-ringring-${deviceId}">Cargando...</div>
    ...
    const detailRingring = document.getElementById(`detail-ringring-${deviceId}`);
    const alarmRef = database.ref(`alarms/${deviceId}`);
    ...
    const detailCallback = (snapshot) => {
	    ...
	    const isRingring = data.ringring === true;
            ...
            if (isRingring) { 
                ...
                detailStatusBox.style.backgroundColor = ''; // Restablece el color de fondo
                detailStatusBox.style.color = ''; // Restablece el color del texto
            }

            detailRingring.textContent = isRingring ? "¡SONANDO!" : "Silencio";
            detailRingring.style.color = isRingring ? "#ff453a" : "#d1d1d6";
            const now = Date.now();
